...
void MainWindow::initWidgets() {
  ui.actionShow_Samples->setChecked(
      global_paraMgr.glarea.getBool("Show Samples"));
  ui.actionShow_Original->setChecked(
      global_paraMgr.glarea.getBool("Show Original"));
  ui.actionShow_Normals->setChecked(
      global_paraMgr.glarea.getBool("Show Normal"));
  ui.actionShow_Neighborhood_Ball->setChecked(
      global_paraMgr.glarea.getBool("Show Radius"));
  ui.actionShow_All_Raidus->setChecked(
      global_paraMgr.glarea.getBool("Show All Radius"));
  ui.actionCull_Points->setChecked(
      global_paraMgr.drawer.getBool("Need Cull Points"));
  ui.actionShow_Individual_Color->setChecked(
      global_paraMgr.drawer.getBool("Show Individual Color"));

  ui.actionShow_Sample_Quads->setChecked(
      paras->glarea.getBool("Show Samples Quad"));
  ui.actionShow_Sample_Dot->setChecked(
      paras->glarea.getBool("Show Samples Dot"));
  ui.actionShow_Sample_Circle->setChecked(
      paras->glarea.getBool("Show Samples Circle"));
  ui.actionShow_Sample_Sphere->setChecked(
      paras->glarea.getBool("Show Samples Sphere"));

  ui.actionShow_Original_Quad->setChecked(
      paras->glarea.getBool("Show Original Quad"));
  ui.actionShow_Original_Dot->setChecked(
      paras->glarea.getBool("Show Original Dot"));
  ui.actionShow_Original_Circle->setChecked(
      paras->glarea.getBool("Show Original Circle"));
  ui.actionShow_Original_Sphere->setChecked(
      paras->glarea.getBool("Show Original Sphere"));

  ui.actionShow_Normal_Color->setChecked(
      paras->drawer.getBool("Use Color From Normal"));
  ui.actionSnap_Each_Iteration->setChecked(
      paras->glarea.getBool("SnapShot Each Iteration"));
  ...
  ui.actionShow_colorful_branches->setChecked(
      paras->drawer.getBool("Use Differ Branch Color"));
}

void MainWindow::initConnect() {
  if (!connect(area, SIGNAL(needUpdateStatus()), this,
  ...
  connect(ui.actionQianSample, SIGNAL(triggered()), this,
          SLOT(getQianSample()));
  ...
  connect(ui.actionReorientate, SIGNAL(triggered()), this,
          SLOT(reorientateNormal()));
  connect(ui.actionWLOP_Setting, SIGNAL(triggered()), this,
          SLOT(showWLopDlg()));
  connect(ui.actionNormal_Setting, SIGNAL(triggered()), this,
          SLOT(showNormalDlg()));
  connect(ui.actionUpsample_Setting, SIGNAL(triggered()), this,
          SLOT(showUpsampleDlg()));

  connect(ui.actionInitial_Sampling_2, SIGNAL(triggered()), this,
          SLOT(initialSampling()));
  connect(ui.actionAuto_Play, SIGNAL(triggered()), this,
          SLOT(autoPlaySkeleton()));
  ...
  connect(ui.actionSkeleton_Setting, SIGNAL(triggered()), this,
          SLOT(showSkeletonDlg()));
  connect(ui.actionRecompute_Quad, SIGNAL(triggered()), this,
          SLOT(recomputeQuad()));

  connect(ui.actionLight_On_Off, SIGNAL(toggled(bool)), this,
          SLOT(lightOnOff(bool)));
  connect(ui.actionShow_Individual_Color, SIGNAL(toggled(bool)), this,
          SLOT(showIndividualColor(bool)));
  connect(ui.actionShow_Samples, SIGNAL(toggled(bool)), this,
          SLOT(showSamples(bool)));
  connect(ui.actionShow_Original, SIGNAL(toggled(bool)), this,
          SLOT(showOriginal(bool)));
  connect(ui.actionShow_Normals, SIGNAL(toggled(bool)), this,
          SLOT(showNormals(bool)));
  connect(ui.actionShow_Neighborhood_Ball, SIGNAL(toggled(bool)), this,
          SLOT(showNeighborhoodBall(bool)));
  connect(ui.actionShow_All_Raidus, SIGNAL(toggled(bool)), this,
          SLOT(showAllNeighborhoodBall(bool)));
  connect(ui.actionCull_Points, SIGNAL(toggled(bool)), this,
          SLOT(cullPoints(bool)));
  connect(ui.actionShow_Normal_Color, SIGNAL(toggled(bool)), this,
          SLOT(showNormalColor(bool)));
  connect(ui.actionSnap_Each_Iteration, SIGNAL(toggled(bool)), this,
          SLOT(setSnapshotEachIteration(bool)));
  connect(ui.actionNo_Snap_Radius, SIGNAL(toggled(bool)), this,
          SLOT(setNoSnapshotWithRadius(bool)));
  connect(ui.actionShow_Skeleton, SIGNAL(toggled(bool)), this,
          SLOT(showSkeleton(bool)));
  connect(ui.actionShow_colorful_branches, SIGNAL(toggled(bool)), this,
          SLOT(showColorfulBranches(bool)));

  connect(sample_draw_type, SIGNAL(triggered(QAction *)), this,
          SLOT(setSmapleType(QAction *)));
  connect(original_draw_type, SIGNAL(triggered(QAction *)), this,
          SLOT(setOriginalType(QAction *)));

  connect(ui.actionSample_Color, SIGNAL(triggered()), this,
          SLOT(sampleColor()));
  connect(ui.actionOriginal_Color, SIGNAL(triggered()), this,
          SLOT(originalColor()));
  connect(ui.actionBackground_Color, SIGNAL(triggered()), this,
          SLOT(backGroundColor()));
  connect(ui.actionAmbient_Color, SIGNAL(triggered()), this,
          SLOT(ambientColor()));
  connect(ui.actionDiffuse_Color, SIGNAL(triggered()), this,
          SLOT(diffuseColor()));
  connect(ui.actionSpecular_Color, SIGNAL(triggered()), this,
          SLOT(specularColor()));
  connect(ui.actionNormal_Color, SIGNAL(triggered()), this,
          SLOT(normalColor()));
  connect(ui.actionFeature_Color, SIGNAL(triggered()), this,
          SLOT(featureColor()));

  connect(ui.actionErase_Pick, SIGNAL(triggered()), this,
          SLOT(removePickPoints()));
...
void MainWindow::updateStatusBar() {
  ...
  int iteration_unm = 0;
  QString running_name =
      global_paraMgr.glarea.getString("Running Algorithm Name");
  ...
  double error = 0;
  // QString running_name = global_paraMgr.glarea.getString("Running Algorithm
  // Name");
  if (running_name == QString("WLOP")) {
...
void MainWindow::openFile() {
  QString file =
      QFileDialog::getOpenFileName(this, "Select a ply file", "", "*.ply");
  if (!file.size())
    return;
...
void MainWindow::openImage() {
  QString file =
      QFileDialog::getOpenFileName(this, "Select a ply file", "", "");
  if (!file.size())
    return;
...
void MainWindow::saveFile() {
  QString file =
      QFileDialog::getSaveFileName(this, "Save samples as", "", "*.ply *.sfl");
  if (!file.size())
    return;
...
void MainWindow::saveSnapshot() { area->saveSnapshot(); }

void MainWindow::saveView() {
  QString file =
      QFileDialog::getSaveFileName(this, "Select a ply file", "", "*.View");
  if (!file.size())
    return;
  area->saveView(file);
}
...
void MainWindow::saveSkel() {
  QString file =
      QFileDialog::getSaveFileName(this, "Save samples as", "", "*.skel");
  if (!file.size())
    return;
...
void MainWindow::runPCA_Normal() {
  ...
  CMesh *samples = area->dataMgr.getCurrentSamples();
  vcg::NormalExtrapolation<vector<CVertex>>::ExtrapolateNormals(
      samples->vert.begin(), samples->vert.end(), knn, -1);
}

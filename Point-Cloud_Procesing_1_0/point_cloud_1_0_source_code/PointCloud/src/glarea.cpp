...
GLArea::GLArea(QWidget *parent)
    : QGLWidget(
          /*QGLFormat(QGL::DoubleBuffer | QGL::DepthBuffer |QGL::SampleBuffers),*/ parent),
...
void GLArea::initLight() {
  ...
  float specular[4] = {color_specular.r, color_specular.g, color_specular.b,
                       1.0};
...
void GLArea::paintGL() {
  paintMutex.lock();
  {
    ...
    double SnapResolutionScale =
        global_paraMgr.glarea.getDouble("Snapshot Resolution");
    if (takeSnapTile) {
      double normal_width =
          global_paraMgr.drawer.getDouble("Normal Line Width");
      double dot_size = global_paraMgr.drawer.getDouble("Sample Dot Size");
      double original_dot_size =
          global_paraMgr.drawer.getDouble("Original Dot Size");

      global_paraMgr.drawer.setValue(
          "Normal Line Width", DoubleValue(normal_width * SnapResolutionScale *
                                           SnapResolutionScale * snapDrawScal));
      global_paraMgr.drawer.setValue(
          "Sample Dot Size", DoubleValue(dot_size * SnapResolutionScale *
                                         SnapResolutionScale * snapDrawScal));
      global_paraMgr.drawer.setValue(
          "Original Dot Size",
          DoubleValue(original_dot_size * SnapResolutionScale *
                      SnapResolutionScale * snapDrawScal));
    }
    ...
    if (!(takeSnapTile && para->getBool("No Snap Radius"))) {
      glDrawer.drawPickPoint(dataMgr.getCurrentSamples(), pickList,
                             para->getBool("Show Samples Dot"));
    }
    ...
    if (para->getBool("Show Radius") &&
        !(takeSnapTile && para->getBool("No Snap Radius"))) {
    ...
    if (takeSnapTile) {
      ...
      global_paraMgr.drawer.setValue(
          "Normal Line Width",
          DoubleValue(normal_width / (SnapResolutionScale *
                                      SnapResolutionScale * snapDrawScal)));
      global_paraMgr.drawer.setValue(
          "Sample Dot Size",
          DoubleValue(dot_size / (SnapResolutionScale * SnapResolutionScale *
                                  snapDrawScal)));
      global_paraMgr.drawer.setValue(
          "Original Dot Size",
          DoubleValue(
              original_dot_size /
              (SnapResolutionScale * SnapResolutionScale * snapDrawScal)));
    }
  }
PAINT_RETURN:
  paintMutex.unlock();
...
void GLArea::runPointCloudAlgorithm(PointCloudAlgorithm &algorithm) {
  ...
  cout << "time used:  " << timeused / double(CLOCKS_PER_SEC) << " seconds."
       << endl;
...
int GLArea::pickPoint(int x, int y, vector<int> &result, int width, int height,
                      bool only_one) {
  ...
  for (long ii = 0; ii < hits; ii++) {
    // H.push_back( std::pair<double,unsigned
    // int>(selectBuf[ii*4+1]/4294967295.0,selectBuf[ii*4+3]));
...
void GLArea::setView() {
  ...
  if (fov == 5)
    cameraDist = 1000; // small hack for orthographic projection where camera
                       // distance is rather meaningless...
  ...
  if (!takeSnapTile) {
    if (fov == 5)
      glOrtho(-viewRatio * fAspect, viewRatio * fAspect, -viewRatio, viewRatio,
              cameraDist - 2.f * clipRatioNear,
...
void GLArea::pasteTile() {
  ...
  if (snapBuffer.isNull())
    snapBuffer =
        QImage(tileBuffer.width() * ss.resolution,
               tileBuffer.height() * ss.resolution, tileBuffer.format());

  uchar *snapPtr = snapBuffer.bits() + (tileBuffer.bytesPerLine() * tileCol) +
                   ((totalCols * tileRow) * tileBuffer.numBytes());
  ...
  if (tileCol >= totalCols) {
    ...
    if (tileRow >= totalRows) {
      QString outfile =
          QString("%1/%2%3.png").arg(ss.outdir).arg(ss.basename).arg("");
...
void GLArea::setTiledView(GLdouble fovY, float viewRatio, float fAspect,
  ...
  if (fovY <= 5) {
    ...
    glFrustum(fLeft + tDimX * tileCol, fLeft + tDimX * (tileCol + 1),
              fBottom + tDimY * tileRow, fBottom + tDimY * (tileRow + 1), zNear,
              zFar);
  }
}
...
void GLArea::runWlop() {
  ...
  bool is_break = false;
  for (int i = 0; i < global_paraMgr.wLop.getDouble("Num Of Iterate Time");
       i++) {
  ...
  if (is_break) {
    global_paraMgr.skeleton.setValue("The Skeletonlization Process Should Stop",
                                     BoolValue(false));
...
void GLArea::runSkeletonization_linear() {
  ...
  para->setValue("Running Algorithm Name",
                 StringValue(skeletonization.getParameterSet()->getString(
                     "Algorithm Name")));
...
void GLArea::runSkeletonization_paralleled() {
  ...
  if (global_paraMgr.skeleton.getBool("Run Auto Wlop One Stage")) {
    global_paraMgr.skeleton.setValue("Run Auto Wlop One Stage",
                                     BoolValue(false));
    emit needUpdateStatus();
    return;
  }
...
void GLArea::runNormalSmoothing() {
  ...
  for (int i = 0; i < global_paraMgr.norSmooth.getInt("Number Of Iterate");
       i++) {
...
void GLArea::saveView(QString fileName) {
  ...
  outfile << rotate_pos[0] << " " << rotate_pos[1] << " " << rotate_pos[2]
          << " " << endl;
  outfile << rotate_normal[0] << " " << rotate_normal[1] << " "
          << rotate_normal[2] << " " << endl;
  ...
  // outfile << global_paraMgr.skeleton.getDouble("Snake Search Max Dist Blue")
  // << endl;
  // outfile << global_paraMgr.skeleton.getDouble("Branch Search Max Dist
  // Yellow") << endl;
  // outfile << global_paraMgr.skeleton.getDouble("Branches Merge Max Dist
  // Orange") << endl;
...
void GLArea::loadView(QString fileName) {
  ...
  global_paraMgr.drawer.setValue("Normal Line Width",
                                 DoubleValue(normal_width));
  global_paraMgr.drawer.setValue("Normal Line Length",
                                 DoubleValue(normal_length));
  ...
  if (!infile.eof()) {
    ...
    global_paraMgr.drawer.setValue("Sample Point Color",
                                   ColorValue(inputColor(infile)));
    global_paraMgr.drawer.setValue("Original Point Color",
                                   ColorValue(inputColor(infile)));
    global_paraMgr.glarea.setValue("Light Ambient Color",
                                   ColorValue(inputColor(infile)));
    global_paraMgr.glarea.setValue("Light Diffuse Color",
                                   ColorValue(inputColor(infile)));
    global_paraMgr.glarea.setValue("Light Specular Color",
                                   ColorValue(inputColor(infile)));
    global_paraMgr.drawer.setValue("Skeleton Bone Color",
                                   ColorValue(inputColor(infile)));
    global_paraMgr.drawer.setValue("Skeleton Node Color",
                                   ColorValue(inputColor(infile)));
    global_paraMgr.drawer.setValue("Skeleton Branch Color",
                                   ColorValue(inputColor(infile)));
    global_paraMgr.drawer.setValue("Normal Line Color",
                                   ColorValue(inputColor(infile)));
  }
  ...
  if (!infile.eof()) {
    ...
    infile >> temp;
    // global_paraMgr.setGlobalParameter("Local Density Radius",
    // DoubleValue(temp));
  }

  if (!infile.eof()) {
    infile >> temp;
    // global_paraMgr.skeleton.setValue("Snake Search Max Dist Blue",
    // DoubleValue(temp));

    infile >> temp;
    // global_paraMgr.skeleton.setValue("Branch Search Max Dist Yellow",
    // DoubleValue(temp));

    infile >> temp;
    // global_paraMgr.skeleton.setValue("Branches Merge Max Dist Orange",
    // DoubleValue(temp));
  }
...
void GLArea::wheelEvent(QWheelEvent *e) {
  ...
  if ((e->modifiers() & Qt::AltModifier) &&
    ...
    if (para->getBool("Show Skeleton") /*&& !dataMgr.isSkeletonEmpty()*/) {
      ...
      global_paraMgr.skeleton.setValue("Branches Merge Max Dist",
                                       DoubleValue(size_temp * change));
    }
  } else if ((e->modifiers() & Qt::ShiftModifier) &&
             (e->modifiers() & Qt::AltModifier)) {
    size_temp =
        global_paraMgr.glarea.getDouble("Radius Ball Transparency") * change;
    global_paraMgr.glarea.setValue("Radius Ball Transparency",
                                   DoubleValue(size_temp));
    ...
    case Qt::ControlModifier:
      ...
      if (para->getBool("Show Skeleton") && !dataMgr.isSkeletonEmpty()) {
        ...
        global_paraMgr.drawer.setValue("Skeleton Node Size",
                                       DoubleValue(size_temp * change));
      } else if (para->getBool("Show Original") &&
                 para->getBool("Show Original Sphere")) {
        size_temp = global_paraMgr.drawer.getDouble("Original Draw Width");
        global_paraMgr.drawer.setValue("Original Draw Width",
                                       DoubleValue(size_temp * change));
      } else if (para->getBool("Show Samples") &&
                 para->getBool("Show Samples Dot") &&
                 para->getBool("Show Original") &&
                 para->getBool("Show Original Dot")) {
        size_temp = global_paraMgr.drawer.getDouble("Sample Dot Size") * change;
        if (size_temp < 1) {
          size_temp = 1;
        }
        global_paraMgr.drawer.setValue("Sample Dot Size",
                                       DoubleValue(size_temp));

        size_temp =
            global_paraMgr.drawer.getDouble("Original Dot Size") * change;
        if (size_temp < 1) {
          size_temp = 1;
        }

        global_paraMgr.drawer.setValue("Original Dot Size",
                                       DoubleValue(size_temp));
      } else if (para->getBool("Show Samples") &&
                 para->getBool("Show Samples Dot")) {
        size_temp = global_paraMgr.drawer.getDouble("Sample Dot Size") * change;
        ...
        global_paraMgr.drawer.setValue("Sample Dot Size",
                 ...
                 para->getBool("Show Samples Quad")) {
        size_temp =
            global_paraMgr.drawer.getDouble("Sample Draw Width") * change;
        if (size_temp < 0) {
          size_temp = 0.001;
        }
        // cout << "draw width: " <<size_temp << endl;
        global_paraMgr.drawer.setValue("Sample Draw Width",
                                       DoubleValue(size_temp));
      } else if (para->getBool("Show Original") &&
                 para->getBool("Show Original Dot")) {
        size_temp =
    ...
    case Qt::ShiftModifier:
      ...
      if (para->getBool("Show Skeleton") && !dataMgr.isSkeletonEmpty()) {
        ...
        global_paraMgr.drawer.setValue("Skeleton Bone Width",
                                       DoubleValue(size_temp * change));
      } else {
...
void GLArea::mousePressEvent(QMouseEvent *e) {
  ...
  if (isDefaultTrackBall()) {
    if (e->button() == Qt::LeftButton)
      trackball.MouseDown(e->x(), height() - e->y(),
                          QT2VCG(e->button(), e->modifiers()));
    ...
    trackball_light.MouseDown(e->x(), height() - e->y(),
                              QT2VCG(e->button(), Qt::NoModifier));
...
void GLArea::mouseReleaseEvent(QMouseEvent *e) {
  ...
  if (e->button() == Qt::LeftButton)
    trackball.MouseUp(e->x(), height() - e->y(),
                      QT2VCG(e->button(), e->modifiers()));
  ...
  if (e->button() == Qt::RightButton) {
    ...
    if ((e->modifiers() & Qt::ControlModifier)) {
      ...
      pickPoint(x3, y3, pickList, abs(x2 - x1), abs(y1 - y2),
                !para->getBool("Multiply Pick Point"));

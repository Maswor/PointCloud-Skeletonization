...
RichParameter *RichParameterSet::findParameter(QString name) const {
  ...
  qDebug("FilterParameter Warning: Unable to find a parameter with name '%s',\n"
         "      Please check types and names of the parameter in the calling "
         "filter",
         qPrintable(name));
  assert(0);
  return 0;
...
bool RichParameterSet::isEmpty() const { return paramList.isEmpty(); }

void RichParameterSet::clear() { paramList.clear(); }
...
void RichParameterCopyConstructor::visit(RichBool &pd) {
  lastCreated =
      new RichBool(pd.name, pd.val->getBool(), pd.pd->defVal->getBool(),
                   pd.pd->fieldDesc, pd.pd->tooltip);
}

void RichParameterCopyConstructor::visit(RichInt &pd) {
  lastCreated = new RichInt(pd.name, pd.val->getInt(), pd.pd->defVal->getInt(),
                            pd.pd->fieldDesc, pd.pd->tooltip);
}

void RichParameterCopyConstructor::visit(RichDouble &pd) {
  lastCreated =
      new RichDouble(pd.name, pd.val->getDouble(), pd.pd->defVal->getDouble(),
                     pd.pd->fieldDesc, pd.pd->tooltip);
}

void RichParameterCopyConstructor::visit(RichString &pd) {
  lastCreated =
      new RichString(pd.name, pd.val->getString(), pd.pd->defVal->getString(),
                     pd.pd->fieldDesc, pd.pd->tooltip);
}

void RichParameterCopyConstructor::visit(RichMatrix44f &pd) {
  lastCreated = new RichMatrix44f(pd.name, pd.val->getMatrix44f(),
                                  pd.pd->defVal->getMatrix44f(),
                                  pd.pd->fieldDesc, pd.pd->tooltip);
}

void RichParameterCopyConstructor::visit(RichPoint3f &pd) {
  lastCreated = new RichPoint3f(pd.name, pd.val->getPoint3f(),
                                pd.pd->defVal->getPoint3f(), pd.pd->fieldDesc,
                                pd.pd->tooltip);
}

void RichParameterCopyConstructor::visit(RichColor &pd) {
  lastCreated =
      new RichColor(pd.name, pd.val->getColor(), pd.pd->defVal->getColor(),
                    pd.pd->fieldDesc, pd.pd->tooltip);
}

void RichParameterCopyConstructor::visit(RichAbsPerc &pd) {
  AbsPercDecoration *dec = reinterpret_cast<AbsPercDecoration *>(pd.pd);
  lastCreated = new RichAbsPerc(pd.name, pd.val->getAbsPerc(),
                                pd.pd->defVal->getAbsPerc(), dec->min, dec->max,
                                pd.pd->fieldDesc, pd.pd->tooltip);
}
...
void RichParameterCopyConstructor::visit(RichEnum &pd) {
  ...
  lastCreated =
      new RichEnum(pd.name, pd.val->getEnum(), pd.pd->defVal->getEnum(),
                   dec->enumvalues, pd.pd->fieldDesc, pd.pd->tooltip);
}
...
void RichParameterCopyConstructor::visit(RichDynamicDouble &pd) {
  DynamicDoubleDecoration *dec =
      reinterpret_cast<DynamicDoubleDecoration *>(pd.pd);
  lastCreated = new RichDynamicDouble(
      pd.name, pd.val->getDynamicDouble(), pd.pd->defVal->getDynamicDouble(),
...
void RichParameterCopyConstructor::visit(RichOpenFile &pd) {
  ...
  lastCreated = new RichOpenFile(pd.name, pd.pd->defVal->getFileName(),
                                 dec->exts, pd.pd->fieldDesc, pd.pd->tooltip);
}

void RichParameterCopyConstructor::visit(RichSaveFile &pd) {
  SaveFileDecoration *dec = reinterpret_cast<SaveFileDecoration *>(pd.pd);
  lastCreated = new RichSaveFile(pd.name, pd.pd->defVal->getFileName(),
                                 dec->ext, pd.pd->fieldDesc, pd.pd->tooltip);
}

//
// void RichParameterXMLVisitor::fillRichParameterAttribute(const QString&
// type,const QString& name,const QString& desc,const QString& tooltip)
// {
...
//
// void RichParameterXMLVisitor::fillRichParameterAttribute(const QString&
// type,const QString& name,const QString& val,const QString& desc,const
// QString& tooltip)
// {
...
//
//
// 	qDebug("    Reading Param with name %s :
// %s",qPrintable(name),qPrintable(type));
//
// 	bool corrconv = false;
// 	if(type=="RichBool")
// 	{
...
// 			return false;
// 		*par = new
// RichBool(name,np.attribute("value")!=QString("false"),desc,tooltip);
// 		return true;
// 	}
//
// 	if(type=="RichInt")
...
// 		{
// 			double val =
// np.attribute(QString("val")+QString::number(i)).toDouble(&corrconv);
// 			if (!corrconv)
// 				return false;
// 			mm.V()[i]=val;
// 		}
...
// 		int enum_card =
// np.attribute(QString("enum_cardinality")).toUInt(&corrconv);
// 		if (!corrconv)
// 			return false;
//
...
// 			return false;
//
// 		*par = new RichDynamicDouble(name, val, min, max, desc,
// tooltip);
// 		return true;
// 	}
//
...
// 		QStringList list = QStringList();
// 		int exts_card =
// np.attribute(QString("exts_cardinality")).toUInt(&corrconv);
// 		if (!corrconv)
// 			return false;
//
...
ParameterDecoration::ParameterDecoration(Value *defvalue,
                                         const QString desc /*= QString()*/,
                                         const QString tltip /*= QString()*/)
    : fieldDesc(desc), tooltip(tltip), defVal(defvalue) {}

ParameterDecoration::~ParameterDecoration() { delete defVal; }

BoolDecoration::BoolDecoration(BoolValue *defvalue,
                               const QString desc /*=QString()*/,
                               const QString tltip /*=QString()*/)
    : ParameterDecoration(defvalue, desc, tltip) {}

IntDecoration::IntDecoration(IntValue *defvalue,
                             const QString desc /*= QString()*/,
                             const QString tltip /*= QString()*/)
    : ParameterDecoration(defvalue, desc, tltip) {}

DoubleDecoration::DoubleDecoration(DoubleValue *defvalue,
                                   const QString desc /*= QString()*/,
                                   const QString tltip /*= QString()*/)
    : ParameterDecoration(defvalue, desc, tltip) {}

StringDecoration::StringDecoration(StringValue *defvalue,
                                   const QString desc /*= QString()*/,
                                   const QString tltip /*= QString()*/)
    : ParameterDecoration(defvalue, desc, tltip) {}

Matrix44fDecoration::Matrix44fDecoration(Matrix44fValue *defvalue,
                                         const QString desc /*= QString()*/,
                                         const QString tltip /*= QString()*/)
    : ParameterDecoration(defvalue, desc, tltip) {}

Point3fDecoration::Point3fDecoration(Point3fValue *defvalue,
                                     const QString desc /*= QString()*/,
                                     const QString tltip /*= QString()*/)
    : ParameterDecoration(defvalue, desc, tltip) {}

ColorDecoration::ColorDecoration(ColorValue *defvalue,
                                 const QString desc /*= QString()*/,
                                 const QString tltip /*= QString()*/)
    : ParameterDecoration(defvalue, desc, tltip) {}

AbsPercDecoration::AbsPercDecoration(AbsPercValue *defvalue,
                                     const double minVal, const double maxVal,
                                     const QString desc /*= QString()*/,
                                     const QString tltip /*= QString()*/)
    : ParameterDecoration(defvalue, desc, tltip), min(minVal), max(maxVal) {}

EnumDecoration::EnumDecoration(EnumValue *defvalue, QStringList values,
                               const QString desc /*= QString()*/,
                               const QString tltip /*= QString()*/)
    : ParameterDecoration(defvalue, desc, tltip), enumvalues(values) {}

DynamicDoubleDecoration::DynamicDoubleDecoration(
    DynamicDoubleValue *defvalue, const double minVal, const double maxVal,
    const QString desc /*= QString()*/, const QString tltip /*= QString()*/)
    : ParameterDecoration(defvalue, desc, tltip), min(minVal), max(maxVal) {}

SaveFileDecoration::SaveFileDecoration(
    FileValue *defvalue, const QString extension /*=QString(".*")*/,
    const QString desc /*= QString()*/, const QString tltip /*= QString()*/)
    : ParameterDecoration(defvalue, desc, tltip), ext(extension) {}

OpenFileDecoration::OpenFileDecoration(FileValue *directorydefvalue,
                                       const QStringList extensions,
                                       const QString desc /*= QString()*/,
                                       const QString tltip /*= QString()*/)
    : ParameterDecoration(directorydefvalue, desc, tltip), exts(extensions) {}

RichParameter::RichParameter(const QString nm, Value *v,
                             ParameterDecoration *prdec)
...
RichBool::RichBool(const QString nm, const bool defval)
    : RichParameter(nm, new BoolValue(defval),
                    new BoolDecoration(new BoolValue(defval), "", "")) {}
RichBool::RichBool(const QString nm, const bool defval, const QString desc)
    : RichParameter(nm, new BoolValue(defval),
                    new BoolDecoration(new BoolValue(defval), desc, "")) {}

RichBool::RichBool(const QString nm, const bool defval,
                   const QString desc /*=QString()*/,
                   const QString tltip /*=QString()*/)
    : RichParameter(nm, new BoolValue(defval),
                    new BoolDecoration(new BoolValue(defval), desc, tltip)) {}

RichBool::RichBool(const QString nm, const bool val, const bool defval,
                   const QString desc /*=QString()*/,
                   const QString tltip /*=QString()*/)
    : RichParameter(nm, new BoolValue(val),
                    new BoolDecoration(new BoolValue(defval), desc, tltip)) {}

void RichBool::accept(Visitor &v) { v.visit(*this); }

bool RichBool::operator==(const RichParameter &rb) {
  return (rb.val->isBool() && (name == rb.name) &&
          (val->getBool() == rb.val->getBool()));
}

RichBool::~RichBool() {}

RichInt::RichInt(const QString nm, const int defval,
                 const QString desc /*=QString()*/,
                 const QString tltip /*=QString()*/)
    : RichParameter(nm, new IntValue(defval),
                    new IntDecoration(new IntValue(defval), desc, tltip)) {}

RichInt::RichInt(const QString nm, const int val, const int defval,
                 const QString desc /*=QString()*/,
                 const QString tltip /*=QString()*/)
    : RichParameter(nm, new IntValue(val),
                    new IntDecoration(new IntValue(defval), desc, tltip)) {}
void RichInt::accept(Visitor &v) { v.visit(*this); }

bool RichInt::operator==(const RichParameter &rb) {
  return (rb.val->isInt() && (name == rb.name) &&
          (val->getInt() == rb.val->getInt()));
}

RichInt::~RichInt() {}

RichDouble::RichDouble(const QString nm, const double defval,
                       const QString desc /*=QString()*/,
                       const QString tltip /*=QString()*/)
    : RichParameter(
          nm, new DoubleValue(defval),
          new DoubleDecoration(new DoubleValue(defval), desc, tltip)) {}

RichDouble::RichDouble(const QString nm, const double val, const double defval,
                       const QString desc /*=QString()*/,
                       const QString tltip /*=QString()*/)
    : RichParameter(
          nm, new DoubleValue(val),
          new DoubleDecoration(new DoubleValue(defval), desc, tltip)) {}
void RichDouble::accept(Visitor &v) { v.visit(*this); }

bool RichDouble::operator==(const RichParameter &rb) {
  return (rb.val->isDouble() && (name == rb.name) &&
          (val->getDouble() == rb.val->getDouble()));
}
...
RichString::RichString(const QString nm, const QString defval,
                       ...
                       const QString tltip /*=QString()*/)
    : RichParameter(
          nm, new StringValue(defval),
          new StringDecoration(new StringValue(defval), desc, tltip)) {}

RichString::RichString(const QString nm, const QString val,
                       const QString defval, const QString desc /*=QString()*/,
                       const QString tltip /*=QString()*/)
    : RichParameter(
          nm, new StringValue(val),
          new StringDecoration(new StringValue(defval), desc, tltip)) {}

RichString::RichString(const QString nm, const QString defval)
    : RichParameter(nm, new StringValue(defval),
                    new StringDecoration(new StringValue(defval), "", "")) {}

RichString::RichString(const QString nm, const QString defval,
                       const QString desc)
    : RichParameter(nm, new StringValue(defval),
                    new StringDecoration(new StringValue(defval), desc, "")) {}

void RichString::accept(Visitor &v) { v.visit(*this); }

bool RichString::operator==(const RichParameter &rb) {
  return (rb.val->isString() && (name == rb.name) &&
          (val->getString() == rb.val->getString()));
}

RichString::~RichString() {}

RichMatrix44f::RichMatrix44f(const QString nm, const vcg::Matrix44f &defval,
                             const QString desc /*=QString()*/,
                             const QString tltip /*=QString()*/)
    : RichParameter(
          nm, new Matrix44fValue(defval),
          new Matrix44fDecoration(new Matrix44fValue(defval), desc, tltip)) {}

RichMatrix44f::RichMatrix44f(const QString nm, const vcg::Matrix44f &val,
                             const vcg::Matrix44f &defval,
                             const QString desc /*=QString()*/,
                             const QString tltip /*=QString()*/)
    : RichParameter(
          nm, new Matrix44fValue(val),
          new Matrix44fDecoration(new Matrix44fValue(defval), desc, tltip)) {}
void RichMatrix44f::accept(Visitor &v) { v.visit(*this); }

bool RichMatrix44f::operator==(const RichParameter &rb) {
  return (rb.val->isMatrix44f() && (name == rb.name) &&
          (val->getMatrix44f() == rb.val->getMatrix44f()));
}

RichMatrix44f::~RichMatrix44f() {}

RichPoint3f::RichPoint3f(const QString nm, const vcg::Point3f defval,
                         const QString desc /*=QString()*/,
                         const QString tltip /*=QString()*/)
    : RichParameter(
          nm, new Point3fValue(defval),
          new Point3fDecoration(new Point3fValue(defval), desc, tltip)) {}

RichPoint3f::RichPoint3f(const QString nm, const vcg::Point3f val,
                         const vcg::Point3f defval,
                         const QString desc /*=QString()*/,
                         const QString tltip /*=QString()*/)
    : RichParameter(
          nm, new Point3fValue(val),
          new Point3fDecoration(new Point3fValue(defval), desc, tltip)) {}
void RichPoint3f::accept(Visitor &v) { v.visit(*this); }

bool RichPoint3f::operator==(const RichParameter &rb) {
  return (rb.val->isPoint3f() && (name == rb.name) &&
          (val->getPoint3f() == rb.val->getPoint3f()));
}
...
RichColor::RichColor(const QString nm, const QColor defval, const QString desc,
                     const QString tltip)
    : RichParameter(nm, new ColorValue(defval),
                    new ColorDecoration(new ColorValue(defval), desc, tltip)) {}

RichColor::RichColor(const QString nm, const QColor val, const QColor defval,
                     const QString desc, const QString tltip)
    : RichParameter(nm, new ColorValue(val),
                    new ColorDecoration(new ColorValue(defval), desc, tltip)) {}

RichColor::RichColor(const QString nm, const QColor defval)
    : RichParameter(nm, new ColorValue(defval),
                    new ColorDecoration(new ColorValue(defval), "", "")) {}

RichColor::RichColor(const QString nm, const QColor defval, const QString desc)
    : RichParameter(nm, new ColorValue(defval),
                    new ColorDecoration(new ColorValue(defval), desc, "")) {}

void RichColor::accept(Visitor &v) { v.visit(*this); }

bool RichColor::operator==(const RichParameter &rb) {
  return (rb.val->isColor() && (name == rb.name) &&
          (val->getColor() == rb.val->getColor()));
}

RichColor::~RichColor() {}

RichAbsPerc::RichAbsPerc(const QString nm, const double defval,
                         const double minval, const double maxval,
                         const QString desc /*=QString()*/,
                         const QString tltip /*=QString()*/)
    : RichParameter(nm, new AbsPercValue(defval),
                    new AbsPercDecoration(new AbsPercValue(defval), minval,
                                          maxval, desc, tltip)) {}

RichAbsPerc::RichAbsPerc(const QString nm, const double val,
                         const double defval, const double minval,
                         const double maxval, const QString desc /*=QString()*/,
                         const QString tltip /*=QString()*/)
    : RichParameter(nm, new AbsPercValue(val),
                    new AbsPercDecoration(new AbsPercValue(defval), minval,
                                          maxval, desc, tltip)) {}
void RichAbsPerc::accept(Visitor &v) { v.visit(*this); }

bool RichAbsPerc::operator==(const RichParameter &rb) {
  return (rb.val->isAbsPerc() && (name == rb.name) &&
          (val->getAbsPerc() == rb.val->getAbsPerc()));
}

RichAbsPerc::~RichAbsPerc() {}

RichEnum::RichEnum(const QString nm, const int defval, const QStringList values,
                   const QString desc /*=QString()*/,
                   const QString tltip /*=QString()*/)
    : RichParameter(
          nm, new EnumValue(defval),
          new EnumDecoration(new EnumValue(defval), values, desc, tltip)) {}

RichEnum::RichEnum(const QString nm, const int val, const int defval,
                   const QStringList values, const QString desc /*=QString()*/,
                   const QString tltip /*=QString()*/)
    : RichParameter(
          nm, new EnumValue(val),
          new EnumDecoration(new EnumValue(defval), values, desc, tltip)) {}
void RichEnum::accept(Visitor &v) { v.visit(*this); }

bool RichEnum::operator==(const RichParameter &rb) {
  return (rb.val->isEnum() && (name == rb.name) &&
          (val->getEnum() == rb.val->getEnum()));
}

RichEnum::~RichEnum() {}

RichDoubleList::RichDoubleList(const QString nm, DoubleListValue *v,
                               DoubleListDecoration *prdec)
    : RichParameter(nm, v, prdec) {}

RichDoubleList::RichDoubleList(const QString nm, DoubleListValue * /*val*/,
                               DoubleListValue *v, DoubleListDecoration *prdec)
    : RichParameter(nm, v, prdec) {}

void RichDoubleList::accept(Visitor &v) { v.visit(*this); }

bool RichDoubleList::operator==(const RichParameter &rb) {
  return (rb.val->isDoubleList() && (name == rb.name) &&
          (val->getDoubleList() == rb.val->getDoubleList()));
}

RichDoubleList::~RichDoubleList() {}

RichDynamicDouble::RichDynamicDouble(const QString nm, const double defval,
                                     const double minval, const double maxval,
                                     const QString desc /*=QString()*/,
                                     const QString tltip /*=QString()*/)
    : RichParameter(nm, new DynamicDoubleValue(defval),
                    new DynamicDoubleDecoration(new DynamicDoubleValue(defval),
                                                minval, maxval, desc, tltip)) {}

RichDynamicDouble::RichDynamicDouble(const QString nm, const double val,
                                     const double defval, const double minval,
                                     const double maxval,
                                     const QString desc /*=QString()*/,
                                     const QString tltip /*=QString()*/)
...
void RichDynamicDouble::accept(Visitor &v) { v.visit(*this); }

bool RichDynamicDouble::operator==(const RichParameter &rb) {
  return (rb.val->isDynamicDouble() && (name == rb.name) &&
          (val->getDynamicDouble() == rb.val->getDynamicDouble()));
}

RichDynamicDouble::~RichDynamicDouble() {}

RichOpenFile::RichOpenFile(const QString nm, const QString directorydefval,
                           const QStringList exts /*= QString("*.*")*/,
                           const QString desc /*=QString()*/,
                           const QString tltip /*=QString()*/)
    : RichParameter(nm, new FileValue(directorydefval),
                    new OpenFileDecoration(new FileValue(directorydefval), exts,
                                           desc, tltip)) {}

void RichOpenFile::accept(Visitor &v) { v.visit(*this); }

bool RichOpenFile::operator==(const RichParameter &rb) {
  return (rb.val->isFileName() && (name == rb.name) &&
          (val->getFileName() == rb.val->getFileName()));
}

RichOpenFile::~RichOpenFile() {}

RichSaveFile::RichSaveFile(const QString nm, const QString filedefval,
                           const QString ext, const QString desc /*=QString()*/,
                           const QString tltip /*=QString()*/)
    : RichParameter(nm, new FileValue(filedefval),
                    new SaveFileDecoration(new FileValue(filedefval), ext, desc,
                                           tltip)) {}

void RichSaveFile::accept(Visitor &v) { v.visit(*this); }

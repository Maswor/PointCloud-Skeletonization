...
ParameterMgr::ParameterMgr(void) {
  init_time++;
  if (init_time > 1) {
    std::cout << "can not init ParameterMgr twice!" << std::endl;
    return;
  }
...
void ParameterMgr::initGlareaParameter() {
  ...
  glarea.addParam(
      new RichPoint3f("Light Position", vcg::Point3f(-4.0, -4.0, -4.0)));
  ...
  // glarea.addParam(new RichPoint3f("Light Position", vcg::Point3f(4.0, 4.0,
  // 4.0)));
  // glarea.addParam(new RichColor("Light Ambient Color", QColor(0.0, 0.0,
  // 0.0)));
  // glarea.addParam(new RichColor("Light Diffuse Color", QColor(204, 204,
  // 204)));
  // glarea.addParam(new RichColor("Light Specular Color", QColor(255, 255,
  // 255)));
...
void ParameterMgr::initSkeletonParameter() {
  ...
  skeleton.addParam(
      new RichBool("The Skeletonlization Process Should Stop", false));
  ...
  skeleton.addParam(
      new RichDouble("Follow Sample Max Angle", 80)); // should add to UI
  skeleton.addParam(new RichDouble("Inactive And Keep Virtual Angle",
                                   60)); // should add to UI
  skeleton.addParam(
      new RichDouble("Save Virtual Angle", 30)); // should add to UI

  skeleton.addParam(
      new RichDouble("Grow Accept Sigma", 0.8)); // should add to UI
  ...
  skeleton.addParam(new RichDouble("Sigma KNN", 6)); // this one is hard to
                                                     // determine, should be
                                                     // small for narrow region,
                                                     // but will lead to
                                                     // unnecessary small
                                                     // branches
  skeleton.addParam(
      new RichDouble("Eigen Feature Identification Threshold", 0.901));
  ...
  skeleton.addParam(new RichDouble("Accept Branch Size",
                                   6)); // important, and hard to determine
  ...
  skeleton.addParam(
      new RichInt("Fix Original Mode", 4)); // 1 for noisy , 4 for clean
  ...
  skeleton.addParam(
      new RichBool("Use Clean Points When Following Strategy", true));
  skeleton.addParam(new RichBool("Use All Connect Strategy", true));
  skeleton.addParam(
      new RichBool("Use Plus Perpendicular Dist Strategy", false));
  skeleton.addParam(new RichBool("Use Kill Too Close Strategy", false));
  skeleton.addParam(
      new RichBool("Use Compute Eigen Ignore Branch Strategy", true));

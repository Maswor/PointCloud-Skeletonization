...
void Skeletonization::runAutoWlopOneStep() {
  ...
  if (iterate_error < para->getDouble("Stop And Grow Error") ||
    ...
    if (para->getBool("Run Auto Wlop One Stage")) {
      para->setValue("The Skeletonlization Process Should Stop",
                     BoolValue(true));
    }
...
void Skeletonization::setFirstIterate() { nTimeIterated = 0; }
...
void Skeletonization::computeAverageTerm(CMesh *samples, CMesh *original) {
  ...
  cout << "Original Size:" << samples->vert[0].original_neighbors.size()
       << endl;
  for (int i = 0; i < samples->vert.size(); i++) {
...
double Skeletonization::wlopIterate() {
  ...
  if (nTimeIterated == 0) {
    ...
    GlobalFun::computeBallNeighbors(
        original, NULL, para->getDouble("CGrid Radius"), original->bbox);
  ...
  for (int i = 0; i < samples->vert.size(); i++) {
    ...
    double mu =
        (mu_length / sigma_length) * (v.eigen_confidence - min_sigma) + mu_min;
...
void Skeletonization::searchNewBranches() {
  ...
  GlobalFun::computeAnnNeigbhors(samples->vert, samples->vert, branch_KNN,
                                 false,
                                 "void Skeletonization::searchNewBranches()");
    ...
    if (new_branch.isEmpty()) {
      ...
      for (int i = 0; i < new_curve.size(); i++) {
        if (!samples->vert[new_curve[i].m_index].is_skel_virtual) {
          samples->vert[new_curve[i].m_index]
              .setSample_JustMoving(); // carefull 1-12
        }
      }
    } else {
      ...
      for (int i = 0; i < new_branch.getSize(); i++) {
        if (new_branch.curve[i].is_skel_virtual) {
          samples->vert[new_branch.curve[i].m_index]
              .setSample_MovingAndVirtual();
...
Branch Skeletonization::searchOneBranchFromDirection(int begin_idx,
                                                     Point3f head_direction) {
  double MAX_Euler_dist = para->getDouble("Snake Search Max Dist Blue");
  double MAX_Perpendicular_dist =
      para->getDouble("Branch Search Max Dist Yellow");
  double MAX_Too_Close_dist = para->getDouble("Combine Too Close Threshold");

  double MAX_Euler_dist2 = MAX_Euler_dist * MAX_Euler_dist;
  double MAX_Perpendicular_dist2 =
      MAX_Perpendicular_dist * MAX_Perpendicular_dist;
  ...
  do {
    ...
    for (int i = 0; i < curr_v.neighbors.size(); i++) {
      ...
      double proj_dist =
          GlobalFun::computeProjDist(curr_v.P(), t.P(), head_direction);
      if (proj_dist < 0) {
        continue;
      }
    ...
    double angle =
        GlobalFun::computeRealAngleOfTwoVertor(head_direction, new_direction);
    if (angle > para->getDouble("Branches Search Angle") ||
        !next_v.is_fixed_sample || next_v.is_skel_branch ||
        next_v.is_skel_virtual) {

      next_v.is_skel_virtual =
          true; // the corresponding sample point is not virtual
...
void Skeletonization::growVirtualTailUntilStop(Branch &branch) {
  ...
  while (is_tail_growing) {
    ...
    //	GlobalFun::computeEulerDistSquare(tail.P(),
    // samples->vert[tail.m_index].P()) > 1e-6)
    //{
    ...
    for (int j = 0; j < v.neighbors.size(); j++) {
      ...
      if (!t.is_skel_ignore) {
        ...
        if (dist2 < follow_dist2) {
          ...
          double proj_dist =
              GlobalFun::computeProjDist(tail.P(), t.P(), tail_direction);
          if (proj_dist < 0) {
            continue;
          }
    ...
    if (min_idx < 0 || candidate_sigma < grow_accept_sigma) {
      ...
      if (angle0 >= 0 && angle1 >= 0 && angle0 < angle_threhold &&
          angle1 < angle_threhold) {
...
void Skeletonization::mergeNearEndsGroup() {
  ...
  for (int i = 0; i < skeleton->branches.size(); i++) {
    ...
    double dist_between_head_tail_2 =
        GlobalFun::computeEulerDistSquare(head, tail);
...
bool Skeletonization::isPosVisited(vector<Point3f> &visited_pts, Point3f p,
...
bool Skeletonization::mergeNearEndsGroupFromP(Point3f p0) {
  ...
  for (int i = 0; i < skeleton->branches.size(); i++) {
    ...
    double dist_tail =
        GlobalFun::computeEulerDistSquare(curve[curve.size() - 1], p0);
  ...
  if (!dangerous_Pts.empty()) {
    ...
    for (int i = 0; i < nearby_Pts.size(); i++) {
      ...
      for (int j = 0; j < dangerous_Pts.size(); j++) {
        double dist_to_dang =
            GlobalFun::computeEulerDistSquare(dangerous_Pts[j], p);
    ...
    if (!exclued_ids.empty()) {
      ...
      for (int i = 0; i < group.size(); i++) {
        if (find(exclued_ids.begin(), exclued_ids.end(), i) ==
            exclued_ids.end()) {
...
void Skeletonization::compareTwoCurvesEnds(Curve &c0, Curve &c1,
                                           double &min_dist, double &angle,
                                           CONNECT_TYPE &C_Type) {
...
double Skeletonization::getRealAngeOfTowCurve(Curve &c0, Curve &c1,
                                              CONNECT_TYPE C_Type) {
  double real_angle = 0;
  ...
  switch (C_Type) {
  case H0_H1:
    // real_angle = GlobalFun::computeRealAngleOfTwoVertor(head0_direction,
    // head1_direction);
  ...
  case T0_H1:
    // real_angle = GlobalFun::computeRealAngleOfTwoVertor(tail0_direction,
    // head1_direction);
...
double Skeletonization::getMaxAngleOfTwoPair(vector<Point3f> &dir0,
                                             vector<Point3f> &dir1) {
  double max_angle = -1;
...
bool Skeletonization::isTowCurvesTheSame(Curve &c0, Curve &c1) {
  ...
  double dist2 = GlobalFun::computeEulerDistSquare(c0[c0.size() - 1].P(),
                                                   c1[c1.size() - 1].P());
  if (dist2 > 1e-6) {
    return false;
  }
...
Branch Skeletonization::mergeTowBranches(Branch &branch0, Branch &branch1,
                                         CONNECT_TYPE C_Type) {
  ...
  case H0_H1: {
    ...
    auto tem_c0 = reverseOneCurve(c0);
    new_curve = combineTwoCurvesInOrder(tem_c0, c1);
  ...
  case H0_T1: {
    ...
    auto temp_rev0 = reverseOneCurve(c0);
    auto temp_rev1 = reverseOneCurve(c1);
    new_curve = combineTwoCurvesInOrder(temp_rev0, temp_rev1);
    new_branch.back_up_head = branch0.back_up_tail;
  ...
  case T0_T1: {
    ...
    auto temp_rev10 = reverseOneCurve(c1);
    new_curve = combineTwoCurvesInOrder(c0, temp_rev10);
    new_branch.back_up_head = branch0.back_up_head;
    new_branch.back_up_tail = branch1.back_up_head;
...
void Skeletonization::labelFixOriginal() {
  ...
  case 2:
    ...
    GlobalFun::computeBallNeighbors(
        samples, original,
        para->getDouble("Local Density Radius") /
            sqrt(para->getDouble("H Gaussian Para")) * pca_para,
        original->bbox);
...
void Skeletonization::updateVirtualHeadFollowSamples(Branch &branch) {
  ...
  if (use_perpend_dist_strategy) {
    for (int i = 0; i < follow_cadidates.size(); i++) {
      ...
      double dist;
      dist = GlobalFun::computeProjPlusPerpenDist(real_head_P, t.P(),
                                                  real_head_direction);
      ...
      if (use_kill_too_close_strategy && dist < too_close_dist &&
          !t.is_skel_virtual) {
        t.is_skel_ignore = true;
        continue;
      }
      ...
      if (dist < follow_dist && dist > too_close_dist) {
        ...
        double angle = GlobalFun::computeRealAngleOfTwoVertor(
            real_head_direction, virtual_direction);
    ...
    for (int i = 0; i < follow_cadidates.size(); i++) {
      ...
      // double dist = GlobalFun::computeEulerDist(real_head_P, t.P());
      if (use_kill_too_close_strategy && dist2 < too_close_dist2 &&
          !t.is_skel_virtual) {
        t.is_skel_ignore = true;
      }
  ...
  if (min_idx >= 0 && (head.m_index != min_idx)) {
    samples->vert[head.m_index]
        .setSample_JustMoving(); // not sure, other curve may using this
...
bool Skeletonization::isVirtualTailHealthy(Branch &branch) {
  ...
  double tail_length =
      GlobalFun::computeEulerDist(c[c.size() - 1].P(), c[c.size() - 2].P());
  ...
  if (angle < save_virtual_angle ||
      (tail_length < follow_dist && angle < bad_virtual_angle)) {
    return true;
  } else {
    return false;
  }
...
void Skeletonization::dealWithVirtualsForTail(Branch &branch) {
  ...
  bool use_virtual_group_strategy =
      para->getBool("Use Virtual Group Merge Strategy");

  // deal with head eat tail problem
  if (branch.isHeadVirtual()) {
    auto temp_gethead = branch.getHead();
    auto temp_gettail = branch.getTail();
    double head_tail_dist2 =
        GlobalFun::computeEulerDistSquare(temp_gethead, temp_gettail);
  ...
  if (isVirtualTailHealthy(
          branch) /*&& para->getBool("Use Nearby Combine Strategy")*/) {
    ...
    for (int i = 0; i < branches.size(); i++) {
      ...
      double dist_tail_2 =
          GlobalFun::computeEulerDistSquare(curve1[curve1.size() - 1], tail_P);
    ...
    // if (branch_ids.size() == 1 && para->getBool("Use Nearby Combine
    // Strategy"))
    //{
    ...
    //    if (angle > para->getDouble("Combine Similar Angle") /*|| dist2 <
    //    1e-6*/)// not sure 1-14
    //    {
  ...
  if (branch.getTailAngle() < para->getDouble("Bad Virtual Angle")) {
    ...
    for (int i = 0; i < branches.size(); i++) {
      ...
      for (int j = 0; j < curve1.size(); j++) {
        ...
        Point3f tail_direction =
            curve0[curve0.size() - 2].P() - curve0[curve0.size() - 3].P();
  ...
  if (branch.isTailVirtual()) {
    ...
    if (tail_angle < 0 || (tail_angle > para->getDouble("Bad Virtual Angle") &&
                           tail_length > MAX_Merge_Dist)) {
      ...
      double too_close_threshold =
          para->getDouble("Combine Too Close Threshold");
      ...
      if (!v.neighbors.empty()) {
        ...
        for (int j = 0; j < v.neighbors.size(); j++) {
          ...
          double proj_dist = GlobalFun::computeProjDist(real_tail_P, t.P(),
                                                        real_tail_direction);
          if (proj_dist < 0) {
            continue;
          }
        ...
        if (avaiable_cnt == 0) {
          // cout << "inactive because of:	" << "No follow candidate
          // around!"
          // << endl;
          is_virtial_tail_bad = true;
        }
...
void Skeletonization::segmentOneCurve(Curve &c) {
  // if (isCurveHeadVirtual(c) || isCurveTailVirtual(c) || !para->getBool("Need
  // Segment Right Away"))
  //{
  //  return;
  //}
  ...
  while (1) {
    ...
    double left_right_dist2 =
        GlobalFun::computeEulerDistSquare(left_next_P, right_P);
  ...
  for (vector<int>::reverse_iterator riter = new_ids_right.rbegin();
       riter != new_ids_right.rend(); ++riter) {
  ...
  //		double dist2 = GlobalFun::computeEulerDistSquare(curr_P,
  // next_P);
  //		if (dist2 > segment_len2)
  //		{
  //			break;
...
void Skeletonization::smoothCurve(Curve &c, double angle_threshold,
                                  int MAX_time) {
  if (c.size() < 3) {
    return;
  }
...
void Skeletonization::subdivisionCurveOnce(Curve &c) {
  ...
  for (int i = 0; i < c.size() - 1; i++) {
    ...
    if (i == 0) {
      ...
      new_v.P() = (c[i - 1].P() * (-1) + c[i].P() * 9 + c[i + 1].P() * 9 -
                   c[i + 2].P()) /
                  16.;
    }
...
void Skeletonization::reconnectSkeleton() {
  ...
  while (1) {
    ...
    for (int i = 0; i < branches.size(); i++) {
      Branch &branch = branches[i];
      auto temp_gethead1 = branch.getHead();
      auto temp_gettail1 = branch.getTail();
      double head_tail_dist2 =
          GlobalFun::computeEulerDistSquare(temp_gethead1, temp_gettail1);
  ...
  while (1) {
    ...
    for (int i = 0; i < branches.size(); i++) {
      ...
      auto temp_gethead2 = branch.getHead();
      auto temp_gettail2 = branch.getTail();
      Point3f &head_P = temp_gethead2;
      Point3f &tail_P = temp_gettail2;
  ...
  while (1) {
    ...
    for (int i = 0; i < branches.size(); i++) {
      ...
      auto temp_gethead3 = branch0.getHead();
      auto temp_gettail3 = branch0.getTail();
      Point3f &head0_P = temp_gethead3;
      Point3f &tail0_P = temp_gettail3;
      ...
      for (int j = 0; j < branches.size(); j++) {
        ...
        auto temp_gethead4 = branch1.getHead();
        auto temp_gettail4 = branch1.getTail();
        Point3f &head1_P = temp_gethead4;
        Point3f &tail1_P = temp_gettail4;

        vector<double> dists_head;
        vector<double> dists_tail;

        dists_head.push_back(
            GlobalFun::computeEulerDistSquare(head0_P, head1_P));
        dists_head.push_back(
            GlobalFun::computeEulerDistSquare(head0_P, tail1_P));
        dists_tail.push_back(
            GlobalFun::computeEulerDistSquare(tail0_P, head1_P));
        dists_tail.push_back(
            GlobalFun::computeEulerDistSquare(tail0_P, tail1_P));

...
{
    cout << endl
	 << "Compute ANN for:	 " << purpose.toStdString() << endl;
    int numKnn = knn + 1;
...
vector<int> GlobalFun::GetRandomCards(int Max)
{
    ...
    nCard.assign(nCard_temp, nCard_temp + Max);
    double low = *std::max_element(nCard.begin(), nCard.end());
    double high = *std::min_element(nCard.begin(), nCard.end());
    for (auto i = 0; i < Max; i++) {
	nCard[i] = Max / (high - low) * (nCard[i] - low);
    }
    std::copy(nCard.begin(), nCard.end(), nCard_final.begin());
    std::cout << "Random value: " << nCard[500] << "\n";
    return nCard_final;
}

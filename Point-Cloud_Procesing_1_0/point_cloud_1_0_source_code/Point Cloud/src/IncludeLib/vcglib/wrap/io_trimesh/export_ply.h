	...
	public:
		...
		for (j = 0, fi = m.face.begin(); fi != m.face.end(); ++fi) {
			...
			} else // ***** ASCII *****
			{
				...
				fprintf(fpout, "9 ");
				for (int z = 0; z < 3; ++z)
				    fprintf(fpout, "%g %g %g ", double(fp->WC(z)[0]) / 255, double(fp->WC(z)[1]) / 255, double(fp->WC(z)[2]) / 255);
			    }

...
void Skeletonization::runAutoWlopOneStep()
{
    ...
    if (iterate_error < para->getDouble("Stop And Grow Error") || iterate_time_in_one_stage > para->getDouble("Max Iterate Time")) {
...
void Skeletonization::dealWithVirtualsForTail(Branch& branch)
{
	...
	double head_tail_dist2 = GlobalFun::computeEulerDistSquare(temp_gethead, temp_gettail);
...
void Skeletonization::reconnectSkeleton()
{
	...
	for (int i = 0; i < branches.size(); i++) {
	    ...
	    double head_tail_dist2 = GlobalFun::computeEulerDistSquare(temp_gethead1, temp_gettail1);
